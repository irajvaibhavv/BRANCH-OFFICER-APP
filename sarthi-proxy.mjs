/*
  ...
  GROQ_API_KEY is optional. Set it and the fast interview tasks (fact extraction, question
  phrasing) run on Llama 3.1 8B instead of Gemini. Leave it unset and everything runs on Gemini
  exactly as before. Neither key may ever carry a VITE_ prefix.
...
http.createServer(async (req, res) => {
  ...
  req.on('end', async () => {
    ...
    try {
      ...
      // The client pre-routes every task (sarthiModel.js); this never re-routes a failed call.
