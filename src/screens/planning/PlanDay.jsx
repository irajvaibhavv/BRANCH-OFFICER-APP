...
import { FiMapPin, FiStar, FiTrendingUp, FiFolder, FiClock, FiLifeBuoy, FiCheck, FiChevronRight, FiMove, FiZap, FiPlus, FiNavigation, FiX, FiSearch } from 'react-icons/fi';
...
/*
  ...
  still pick any DSA directly. Then: fastest route, or drag to set their own priority → "Start my day".
*/
...
export default function PlanDay() {
  ...
  const { dsas, loanFiles, todayVisits, planDay } = useAppState();
  const { toast } = useToast();
  const [step, setStep] = useState(0); // 0 choose DSAs · 1 route
  const [criteria, setCriteria] = useState([]);
  const [query, setQuery] = useState('');
  const [picks, setPicks] = useState([]); // dsa ids, in the order added
  ...
  const togglePick = (id) => setPicks((p) => (p.includes(id) ? p.filter((x) => x !== id) : p.length >= 6 ? (toast('Max 6 stops in a day', 'warning', 1400), p) : [...p, id]));
  const addMatches = () => setPicks((p) => [...p, ...matches.map((m) => m.d.id).filter((id) => !p.includes(id))].slice(0, 6));

  const goRoute = () => { setOrder(optimise(picks.map((id) => dsas.find((d) => d.id === id)))); setCustom(false); setStep(1); };

  // ETA chain for the current order
  let prev = START, t = DAY_START, total = 0;
  ...
  const start = () => {
    ...
    toast(`Day planned · ${legs.length} DSA visits`, 'success');
  ...
  return (
    <Page mode="slide" style={{ paddingBottom: 'calc(var(--tabbar-safe) + 104px)' }}>
      <TopBar
        ...
        title={step === 0 ? 'Visit DSAs' : custom ? 'Your priority order' : 'Fastest route'}
        subtitle={step === 0 ? `${picks.length} of 6 picked` : `${order.length} stops · ${total.toFixed(0)} km`}
        hideBell
      />
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 0 && (
          <motion.div key="s0" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.2 }}>
            ...
            {pool.length === 0 && <p className="hint" style={{ marginTop: 16, textAlign: 'center' }}>No DSA matches "{query}".</p>}
        ...
        {step === 1 && (
          <motion.div key="s1" initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.2 }}>
            <div className={styles.seg}>
              <button className={`${styles.segBtn} ${!custom ? styles.segOn : ''}`} onClick={() => { setOrder(optimise(order)); setCustom(false); }}><FiZap size={14} /> Fastest route</button>
            ...
            <p className="hint" style={{ marginBottom: 10 }}>{custom ? 'Drag the handle to set the order you want to visit in.' : 'Ordered for least travel from the branch. Switch to "My priority" to reorder.'}</p>
                  ...
                  <Card padding={14} noChevron className={custom ? styles.dragCard : ''}>
                    <div className="row" style={{ gap: 12 }}>
                      ...
                      <div className="grow" style={{ minWidth: 0 }}>
                        ...
                        <div className="hint truncate">{leg.d.location} · {leg.dist.toFixed(1)} km from {i === 0 ? 'branch' : 'previous'}</div>
      ...
      <div className={styles.planBar}>
        {step === 0 && <Button full disabled={picks.length === 0} icon={<FiNavigation size={18} />} onClick={goRoute}>{picks.length === 0 ? 'Add DSAs to plan' : `Build route · ${picks.length} stop${picks.length === 1 ? '' : 's'}`}</Button>}
        {step === 1 && <Button full icon={<FiCheck size={18} />} onClick={start}>Start my day · {order.length} visits</Button>}
      </div>
    </Page>
  );
}
