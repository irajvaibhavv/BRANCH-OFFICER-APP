/*
  SARTHI — scripted demo mode.

  Used when the Gemini proxy is not reachable (no key set, proxy not running, no network), so a
  client demo always has a complete end-to-end flow. The questions are built from the same
  grounded JSON the live agent gets, the claims come from what the borrower actually says, and the
  report is assembled by the same verifier + eligibility math. Only the wording is pre-written.
*/
import { lookupLocation, lookupBusiness, lookupRiskPattern } from './sarthiTools';
import { identitySummary } from './sarthiId';
import { toNumber } from './sarthiVerifier';
import { formatINR } from './formatters';
import { PHOTO_ASKS } from './sarthiPhoto';

const fmt = (n) => formatINR(n, { compact: false });

/**
 * The question sequence for a case. Steps with a claimType turn the borrower's next answer
 * into a claim record, exactly like Agent 1's ```claim``` blocks do in live mode.
 *
 * Every step carries two strings: `text` is the Hinglish shown as the caption, `speech` is the
 * same question in Devanagari. A Hindi TTS voice reading romanized Hindi ("poochhunga", "kitni")
...
export function buildScript(c) {
  ...
  // Photographs, asked while the phone is still in their hands.
  ...
  // Asked again, in different words, far from the first ask — the coaching check.
  steps.push({
...
/* ============================================================ intake: a walk-in, start to finish

   Nothing is known about this person, so Sarthi builds the file by asking. Each step names the
   field it fills; the screen writes the answer straight into the record as it goes. Identity
   numbers are typed rather than spoken — speech recognition mangles digits and PAN letters.
*/
...
/** What they say they do, mapped onto a trade we hold questions for. No match = no trade questions. */
...
/** Turn one borrower answer into a claim record, keeping their exact words as verbatim. */
